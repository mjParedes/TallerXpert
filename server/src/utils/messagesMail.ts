...
export async function getMerchantOrder(id: number) {
    ...
    // Enviar correo electrónico con la URL de respuesta y archivo adjunto
    await sendEmailWithAttachment(externalReference, orderStatus)
...
async function sendEmailWithAttachment(externalReference: string, orderStatus: string) {
    // Configurar el contenido del correo electrónico
    const mailOptions = {
        from: 'tallerXpert@gmail.com',
        to: 'cliente@gmail.com',
        subject: 'Resultado de la orden en MercadoPago',
        text: `La orden con referencia externa ${externalReference} tiene el estado: ${orderStatus}. Visita la URL de MercadoPago para más detalles: https://url-de-mercadopago.com`,
        attachments: [
            {
                filename: 'documento.pdf', // Nombre del archivo adjunto
                path: '/ruta/al/archivo/documento.pdf', // Ruta al archivo PDF 
                contentType: 'application/pdf', // Tipo MIME del archivo adjunto (pdf, doc, xml)
            },
        ],
    }
