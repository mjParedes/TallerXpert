...
import { getSupplierById } from '../supplierRequest'
...
//   cuit: '30838187796',
//   description:
//     'Lorem ipsum dolor sit amet consectetur, adipisicing elit. Saepe labore quia beatae unde aliquam perferendis mollitia nihil perspiciatis aut voluptas!'
// }
...
export default function SupplierDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const dataSupplier = async () => {
      try {
        const data = await getSupplierById(session, params.id)
        setSupplier(data)
        setLoading(false)
      } catch (error) {
    ...
    <div>
      <div className='flex'>
        ...
        >
          ...
          <defs>
            <path id='clip0_536_586'>
              <rect width='32' height='32' fill='white' />
            </path>
          </defs>
        </svg>

        <h2 className='flex items-center gap-2 text-2xl font-bold mb-7'>
